...
import { elementHasClass, elementHasName } from '@/app/tru-seo/parsers/parse/build/private/filterHelpers'
import filterTree from '@/app/tru-seo/parsers/parse/build/private/filterTree'
...
const getSentenceBeginning = (sentence, firstWordExceptions, secondWordExceptions) => {
	...
	if (firstWordExceptions.includes(sentenceBeginning) && 1 < words.length) {
		sentenceBeginning += ' ' + words[1].toLowerCase()
		if (secondWordExceptions && secondWordExceptions.includes(words[1])) { // NOSONAR: researcher.getConfig() returns false for langs without this list; optional-chain would throw (S6582).
