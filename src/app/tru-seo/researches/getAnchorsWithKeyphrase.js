...
import filterWordsFromArray from '@/app/tru-seo/languageProcessing/helpers/word/filterWordsFromArray'
import { findTopicFormsInString } from '@/app/tru-seo/languageProcessing/helpers/match/findKeywordFormsInString'
import getWords from '@/app/tru-seo/languageProcessing/helpers/word/getWords'
import matchTextWithArray from '@/app/tru-seo/languageProcessing/helpers/match/matchTextWithArray'
import parseSynonyms from '@/app/tru-seo/languageProcessing/helpers/sanitize/parseSynonyms'
import processExactMatchRequest from '@/app/tru-seo/languageProcessing/helpers/match/processExactMatchRequest'
import urlHelper from '@/app/tru-seo/languageProcessing/helpers/url/url.js'
import { WORD_BOUNDARY_WITH_HYPHEN, WORD_BOUNDARY_WITHOUT_HYPHEN } from '@/app/tru-seo/config/wordBoundariesWithoutPunctuation'
...
function getAnchorsLinkingToSelf (anchors, siteUrlOrDomain) {
	...
	return  anchors.filter((_anchor, index) => !anchorsLinkingToSelf[index])
}
...
function getAnchorsContainingTopic (anchors, topicForms, locale, matchWordCustomHelper) {
	const anchorsContainingTopic = anchors.map(function (anchor) {
		...
		return 100 === findTopicFormsInString(topicForms, anchorText, true, locale, matchWordCustomHelper).percentWordMatches
	})

	return anchors.filter((_anchor, index) => anchorsContainingTopic[index])
}
...
function getAnchorsWithSameTextAsTopic (anchors, topicForms, locale, customHelpers, exactMatchRequest, areHyphensWordBoundaries) {
	...
	anchors.forEach(function (currentAnchor) {
        ...
        */
		...
		if (0 < filteredAnchorWords.length) {
		...
		exactMatchRequest.forEach(request => {
			...
			if (request.exactMatchRequested &&
				anchorWords.every(anchorWord => request.keyphrase.toLowerCase().includes(anchorWord.toLowerCase()))) {
				anchorsContainedInTopic.push(true)
			}
		})

		// Check if every word in the anchor text is also present in the keyphrase/synonym.
		for (const topicForm of keyphraseAndSynonymsWords) {
			if (anchorWords.every(anchorWord => 0 < matchTextWithArray(anchorWord, topicForm, locale, matchWordCustomHelper).count)) {
				anchorsContainedInTopic.push(true)
				break
			}
		}
	...
	return anchors.filter((_anchor, index) => anchorsContainedInTopic[index])
}
...
export default function (paper, researcher) {
	...
	let anchors = paper.getTree().findAll(treeNode => 'a' === treeNode.name)
	/*
	 * We get the site's URL (e.g., https://example.com) or domain (e.g., example.com) from the paper.
