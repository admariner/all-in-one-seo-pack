...
import wordMatch from '@/app/tru-seo/languageProcessing/helpers/match/matchTextWithWord.js'
import { findTopicFormsInString } from '@/app/tru-seo/languageProcessing/helpers/match/findKeywordFormsInString.js'
import { stemPrefixedFunctionWords } from '@/app/tru-seo/languageProcessing/helpers/morphology/stemPrefixedFunctionWords.js'
...
import { WORD_BOUNDARY_WITH_HYPHEN } from '@/app/tru-seo/config/wordBoundariesWithoutPunctuation'
...
function findExactMatch (matchesObject, keyphrase, result, prefixedFunctionWordsRegex, title, locale) {
	...
	const arrays = splitKeyphrase.map(
		word => matchedPrefixedFunctionWords
			...
			.filter(keyphraseWord => 0 < wordMatch(title, keyphraseWord, locale, false).count)
	)

	// Check if any of the arrays is empty, which means that the keyphrase is not found in the SEO title.
	if (!arrays.find(array => 0 === array.length)) {
...
function checkIfAllWordsAreFound (paper, researcher, keyword, result, prefixedFunctionWordsRegex) {
	...
	if (100 === separateWordsMatched.percentWordMatches) {
		/*
...
const findKeyphraseInSEOTitle = function (paper, researcher) {
	...
	if (0 < keywordMatched.count && !prefixedFunctionWordsRegex) {
		result.exactMatchFound = true
		result.allWordsFound = true
