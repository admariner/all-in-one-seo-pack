...
import { createShortcodeTagsRegex } from '@/app/tru-seo/languageProcessing/helpers'
...
export default function (paper, researcher) {
	...
	paragraphs = paragraphs.filter(paragraph => {
		...
		return !(paragraph.isImplicit && 'figcaption' === parentNode?.name)
	})
	...
	paragraphs = paragraphs.filter(paragraph => {
		return !createShortcodeTagsRegex([ 'caption', 'gallery', 'embed', 'playlist' ]).test(paragraph.childNodes?.[0]?.value)
	})
	...
	const startOffset = firstParagraph?.sourceCodeLocation.startOffset
	...
	if (!isEmpty(sentences)) {
		const firstResultSentence = sentences
			...
			.find(resultSentence => 100 === resultSentence.percentWordMatches)
		...
		if (100 === resultParagraph.percentWordMatches) {
			result.foundInParagraph = true
