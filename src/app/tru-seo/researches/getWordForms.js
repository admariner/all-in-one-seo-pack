...
import { collectStems, StemOriginalPair } from '@/app/tru-seo/languageProcessing/helpers/morphology/buildTopicStems'

import escapeRegExp from 'lodash-es/escapeRegExp'
import flattenDeep from 'lodash-es/flattenDeep'
import getAllWordsFromPaper from '@/app/tru-seo/languageProcessing/helpers/morphology/getAllWordsFromPaper'
...
function extractStems (keyphrase, synonyms) {
	const keyphraseStemsOnly = 0 === keyphrase.stemOriginalPairs.length
		? []
		: keyphrase.getStems()

	const synonymsStemsOnly = 0 === synonyms.length
		? []
...
function constructTopicPhraseResult (topicPhrase, paperWordsGroupedByStems, createBasicWordForms) {
	...
	if (0 === topicPhrase.stemOriginalPairs.length) {
		return []
	}
...
function getWordForms (keyphrase, synonyms, allWordsFromPaper, functionWords, stemmer, createBasicWordForms, areHyphensWordBoundaries) {
	...
	if (0 === keyphraseStemmed.stemOriginalPairs.length && 0 === synonymsStemmed.length) {
		return new Result()
	}
	...
	if (allTopicPhrases.every(topicPhrase => true === topicPhrase.exactMatch)) {
		return new Result(
	...
	const paperWordsGroupedByStems = paperWordsWithStems.reduce(function (accumulator, stemOriginalPair) {
		...
		if (0 === accumulator.length || lastItem.stem !== stemOriginalPair.stem) {
