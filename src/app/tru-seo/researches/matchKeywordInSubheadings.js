import { getSubheadingContentsAll } from '@/app/tru-seo/languageProcessing/helpers/html/getSubheadings'
import stripSomeTags from '@/app/tru-seo/languageProcessing/helpers/sanitize/stripNonTextTags'
...
const numberOfSubheadingsReflectingTopic = function (topicForms, subheadings, useSynonyms, locale, functionWords, matchWordCustomHelper) {
	return subheadings.filter(subheading => {
		...
		if (0 === functionWords.length) {
			return 100 === matchedTopicForms.percentWordMatches
		}
		return 50 < matchedTopicForms.percentWordMatches
	}).length
}
...
export default function matchKeywordInSubheadings (paper, researcher) {
	...
	const subheadings = getSubheadingContentsAll(text)
