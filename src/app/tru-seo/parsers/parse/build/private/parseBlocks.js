...
const blockTokenizer = /<!--\s+wp:([a-z][a-z0-9_-]*\/)?([a-z][a-z0-9_-]*)\s+({(?:(?=([^}]+|}+(?=})|(?!}\s+\/?-->)[^])*)(?:\/)?|[^]*?)}\s+)?(\/)?-->/g
...
export function updateBlocksOffset (blocks, text) {
	...
	blocks.forEach((currentBlock, index) => {
		...
		if ('core/freeform' === currentBlock.name) {
		...
		if (currentBlock.innerBlocks && 0 < currentBlock.innerBlocks.length) {
...
function updateClientIdAndAttrIdForSubtree (rootNode, blocks, clientId) {
	...
	(rootNode.childNodes || []).forEach((node) => {
		if (node.attributes?.id) {
			 ...
			 * This step is specifically for parsing blocks with sub-blocks (e.g., FAQ blocks).
			 *
			 * For these blocks, if a node has attribute with 'id' key, this means that this node represents a sub-block.
			 * A sub-block has four child nodes:
			 * 1. The first child node of the sub-block, represents the first section of that sub-block.
			 *    - For example, in FAQ block, the first child node would represent the "question" section.
			 ...
			 *    - For example, in FAQ block, the second child node would represent the "answer" section.
			 * 4. The fourth child node of the sub-block, only contains a new line.
			 */
			if (node.childNodes && 3 < node.childNodes.length) {
