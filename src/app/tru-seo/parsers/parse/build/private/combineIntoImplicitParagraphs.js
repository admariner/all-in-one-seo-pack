...
function isInterElementWhitespace (node) {
	return '#text' === node.name && node.value?.match(/^\s+$/g)
}
...
function hasChildren (node) {
	return node && 0 < node.childNodes.length
}
...
const isPartOfDoubleBreak = (currentNode, prevNode, nextNode) => {
	const prevNodeIsBreak = 'br' === prevNode?.name
	const nextNodeIsBreak = 'br' === nextNode?.name
	return 'br' === currentNode.name && (prevNodeIsBreak || nextNodeIsBreak)
}
