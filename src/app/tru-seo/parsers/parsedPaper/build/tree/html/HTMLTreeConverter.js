...
class HTMLTreeConverter {
	/**
	 * Converts the parse5 tree to a structured tree.
	 *
	...
	/**
	 * Converts the tree from a parse5 implementation to a structured tree.
	 *
	 * @param {Object}                                parse5Tree    The parse5 tree to convert.
	...
	_createChild (parse5Node, parentNode) {
		...
		} else if ('ol' === nodeType || 'ul' === nodeType) {
			// List node.
			child = new List('ol' === nodeType, parse5Node.sourceCodeLocation)
	...
	_parseAttributes (parse5attributes) {
		if (parse5attributes && 0 < parse5attributes.length) {
