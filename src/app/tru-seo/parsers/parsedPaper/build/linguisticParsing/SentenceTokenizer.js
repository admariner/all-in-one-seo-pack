...
export default class SentenceTokenizer {
	...
	trimWhiteSpaceAtStart (sentence) {
		const whiteSpaceLength = whiteSpaceStartRegex.exec(sentence.text)[0].length
	...
	trimWhiteSpaceAtEnd (sentence) {
		const whiteSpaceLength = whiteSpaceEndRegex.exec(sentence.text)[0].length
	...
	/**
	 * Determines whether a new sentence should start after a sentence-terminating token.
	 *
	 * @since 5.0.0
	 *
	 * @param {Object} nextToken         The token immediately after the terminator.
	 * @param {Object} secondToNextToken The token after that one.
	 * @param {number} nextStartIndex    Index into the next two characters to use as the candidate sentence beginning (0 for block-end, 1 for full-stop).
	 *
	 * @returns {boolean} True when a new sentence should start.
	 */
	shouldStartNewSentenceAfter (nextToken, secondToNextToken, nextStartIndex) {
		const nextCharacters = this.getNextTwoCharacters([ nextToken, secondToNextToken ])
		const hasNextSentence = 2 <= nextCharacters.length
		...
		const nextSentenceStart = hasNextSentence ? nextCharacters[nextStartIndex] : ''
		return (hasNextSentence && this.isValidSentenceBeginning(nextSentenceStart)) || this.isSentenceStart(nextToken)
	}
	...
	getSentencesFromTokens (tokenArray) {
		...
		tokenArray.forEach((token, i) => {
			...
			switch (token.type) {
				...
				case 'full-stop':
					currentSentence.appendText(token.src)
					if (this.shouldStartNewSentenceAfter(nextToken, secondToNextToken, 1)) {
						tokenSentences.push(currentSentence)
						currentSentence = new Sentence('')
					}
					break
				...
				case 'block-end':
					currentSentence.appendText(token.src)
					if (this.shouldStartNewSentenceAfter(nextToken, secondToNextToken, 0)) {
						tokenSentences.push(currentSentence)
						currentSentence = new Sentence('')
					}
					break
