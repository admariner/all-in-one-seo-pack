...
export const deConstructAnchor = function (anchor) {
	...
	const [ , openTag, content ] = anchorDeconstructionRegex.exec(anchor)
	return {
		openTag : openTag,
		content : content
	}
...
export const collectMarkingsInSentence = function (sentence, wordsFoundInSentence, matchWordCustomHelper) {
	...
	wordsFoundInSentence.forEach(word => {
		...
		const matchedSingleQuotes = word.match(SINGLE_QUOTES_REGEX) // NOSONAR: SINGLE_QUOTES_REGEX has /g; .forEach iterates all match strings (S6594).
	 ...
	 */
	return (markup.replace(/<\/truseomark> <truseomark class='truseo-text-mark'>/ig, ' '))
}
...
export function markWordsInSentences (wordsToMark, sentences, locale, matchWordCustomHelper) {
	...
	sentences.forEach(function (sentence) {
		...
		if (0 < wordsFoundInSentence.length) {
