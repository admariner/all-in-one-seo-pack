...
/**
 * Gets all level 2 through 6 subheadings from the text and returns these in an array.
 * H1 is treated as the main heading, not a subheading, so it is excluded.
 *
 * @param {string} text The text to return the headings from.
 *
 * @returns {Array<string[]>} Matches of subheadings in the text, first key is everything including tags,
 *                            second is the heading level, third is the content of the subheading.
 */
function getSubheadingsAll (text) {
	const subheadings = []
	const regex = /<h([2-6])(?:[^>]+)?>(.*?)<\/h\1>/ig
	let match
...
/**
 * Gets the content of subheadings h2 through h6 in the text.
 *
 * @param {string} text The text to get the subheading contents from.
 *
 * @returns {string[]} A list of all the subheadings with their content.
 */
function getSubheadingContentsAll (text) {
	const subheadings = getSubheadingsAll(text)
