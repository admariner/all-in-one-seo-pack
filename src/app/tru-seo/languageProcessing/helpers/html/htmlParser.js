...
/**
 * The attributes that should be allowed.
 * @type {string[]}
 */
const ALLOWED_ATTRIBUTES = [ 'src', 'alt', 'href' ]
...
const parser = new Parser({
	...
	onopentag : function (tagName, nodeValue) {
		...
		const nodeValueType = Object.keys(nodeValue).filter(attr => ALLOWED_ATTRIBUTES.includes(attr))
		let nodeValueString = ''
	...
	onclosetag : function (tagName) {
		if (1 === ignoreStack.length && ignoreStack[0] === tagName) {
			inIgnorableBlock = false
			ignoreStack = []
			return
		}
...
export default function (text, ignoredClassesException) {
	...
	parser.onopentag = function (tagName, nodeValue) {
		if (inIgnorableBlock) {
			ignoreStack.push(tagName)
			return
		}
