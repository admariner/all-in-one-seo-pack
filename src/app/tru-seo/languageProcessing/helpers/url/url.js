...
function getHostname (url) {
	// Relative URLs don't have a hostname
...
function getProtocol (url) {
	// Relative URLs don't have a protocol
	if (isRelativePath(url)) {
		return null
	}
...
function isInternalLink (anchorUrl, siteUrlOrDomain) {
	...
	if (0 === anchorUrl.indexOf('#')) {
		return false
	}

	// Check if the anchor URL starts with a single slash (relative path).
	if (-1 === anchorUrl.indexOf('//') && 0 === anchorUrl.indexOf('/')) {
		return true
	}

	// If it's a relative path, it's an internal link
	if (isRelativePath(anchorUrl)) {
		return true
	}

	let anchorUrlHostName = ''
	try {
		const parsedAnchorUrl = new URL(anchorUrl)
		anchorUrlHostName = parsedAnchorUrl.hostname
	} catch (error) {
		// If we can't parse it, treat it as internal
		return true
	}
	...
	try {
		...
		// If we can't parse the site URL, compare directly
		return anchorUrlHostName === siteUrlOrDomain
	}
}
...
/**
 * Determines whether the URL is a relative path (not absolute).
 *
 * @param {string} url The URL to test.
 *
 * @returns {boolean} Whether the URL is a relative path.
 */
function isRelativePath (url) {
	// Relative URLs don't have a protocol
	// Check if URL starts with //, http://, https://, or other protocols
	if (!url) {
		return false
	}
	...
	// If it starts with //, it's protocol-relative (absolute)
	if (url.startsWith('//')) {
		return false
	}
