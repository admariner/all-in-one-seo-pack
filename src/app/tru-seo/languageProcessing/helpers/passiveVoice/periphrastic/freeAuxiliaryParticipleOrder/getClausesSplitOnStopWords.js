...
function getClausesSplitOnStopWords (sentence, options) {
	...
	if (null === sentence.match(auxiliaryRegex)) { // NOSONAR: auxiliaryRegex has the /g flag; .exec() would mutate lastIndex across calls (S6594).
		return []
	}

	let clauses
	const stopwords = sentence.match(options.regexes.stopwordRegex) || [] // NOSONAR: stopwordRegex has /g; .match() returns all match strings used by splitOnStopWords (S6594).
	...
	if ('undefined' !== typeof (options.regexes.stopCharacterRegex) && 1 === clauses.length) {
