...
const getSentenceBreakers = function (sentence, options) {
	...
	let auxiliaryIndices = getIndicesOfList(options.auxiliaries, sentence),
		stopwordIndices = getIndicesOfList(options.stopwords, sentence)
	...
	if (options.otherStopWordIndices && 0 < options.otherStopWordIndices.length) {
		stopwordIndices = stopwordIndices.concat(options.otherStopWordIndices)
	}

	if ('undefined' !== typeof regexes.directPrecedenceExceptionRegex) {
	...
	if ('undefined' !== typeof regexes.elisionAuxiliaryExceptionRegex) {
	...
	const totalIndices = filterIndices(auxiliaryIndices.concat(stopwordIndices, stopCharacterIndices))
...
const getAuxiliaryMatches = function (sentencePart, regexes) {
	...
	let auxiliaryMatches = sentencePart.match(auxiliaryRegex) || [] // NOSONAR: auxiliaryRegex has /g; we need all match strings, not a single capture-group result from .exec() (S6594).

	if ('undefined' !== typeof directPrecedenceExceptionRegex || 'undefined' !== typeof followingAuxiliaryExceptionRegex) {
		...
		if ('undefined' !== typeof directPrecedenceExceptionRegex) {
...
const getClauses = function (sentence, options) {
	...
	if (null === sentence.match(auxiliaryRegex)) { // NOSONAR: auxiliaryRegex has /g; .exec() would mutate lastIndex across calls (S6594).
		return clauses
	}
