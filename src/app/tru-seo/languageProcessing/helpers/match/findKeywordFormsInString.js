...
const findWordFormsInString = function (keywordForms, text, locale, matchWordCustomHelper) {
	...
	for (let i = 0; i < wordNumber; i++) {
		...
		foundWords[i] = 0 < matchedKeyphrase.count ? 1 : 0
	...
	result.position = 0 === positions.length ? -1 : Math.min(...positions)
...
const findTopicFormsInString = function (topicForms, text, useSynonyms, locale, matchWordCustomHelper) {
	...
	if (100 === result.percentWordMatches || false === useSynonyms || isEmpty(topicForms.synonymsForms)) {
		return result
	}
