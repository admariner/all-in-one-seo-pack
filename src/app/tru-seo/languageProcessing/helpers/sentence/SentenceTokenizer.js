...
import { normalize as normalizeQuotes } from '@/app/tru-seo/languageProcessing/helpers/sanitize/quotes.js'

import abbreviations from '@/app/tru-seo/languages/en/config/abbreviations'

import createRegexFromArray from '@/app/tru-seo/languageProcessing/helpers/regex/createRegexFromArray'
import wordBoundaries from '@/app/tru-seo/config/wordBoundaries'
...
export default class SentenceTokenizer {
	...
	isPartOfPersonInitial (token, previousToken, nextToken, secondToNextToken) {
		return (!isUndefined(token) &&
			...
			'full-stop' === token.type &&
			'sentence' === previousToken.type &&
			lastCharacterPartOfInitialsRegex.test(previousToken.src) &&
			'sentence' === nextToken.type &&
			1 === nextToken.src.trim().length &&
			'full-stop' === secondToNextToken.type
		)
	}
	...
	endsWithAbbreviation (currentSentence) {
		const matchedAbbreviations = Array.from(currentSentence.matchAll(abbreviationsRegex), m => m[0])

		if (!matchedAbbreviations.length) {
			return false
		}
	...
	getSentencesFromTokens (tokenArray, trimSentences = true) {
		...
		do {
			...
			if (firstToken && lastToken && 'html-start' === firstToken.type &&
				'html-end' === lastToken.type && this.isValidTagPair(firstToken, lastToken)) {
