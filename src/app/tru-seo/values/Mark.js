...
import { isUndefined } from '@/app/tru-seo/helpers'
...
class Mark {
	 ...
	 */
	getBlockClientId () {
		return this._properties.position?.clientId
	}
	 ...
	 */
	getBlockAttributeId () {
		return this._properties.position?.attributeId
	}

	/**
	 * Checks if the mark object is intended for the first section of a sub-block.
	 * This method will be used only for blocks where each block consists of sub-blocks
	 * with two sections.
	 *
	 * @returns {boolean} Whether the mark object is intended for the first section of a sub-block.
	 */
	isMarkForFirstBlockSection () {
		return this._properties.position?.isFirstSection
	}
	...
	getBlockPositionStart () {
		return this._properties.position?.startOffsetBlock
	}
	 ...
	 */
	getBlockPositionEnd () {
		return this._properties.position?.endOffsetBlock
	}
	 ...
	 */
	isValid () {
		if (!isUndefined(this.getPositionStart()) && 0 > this.getPositionStart()) {
			throw new RangeError('positionStart should be larger or equal than 0.')
		}
		if (!isUndefined(this.getPositionEnd()) && 0 >= this.getPositionEnd()) {
