...
class SubheadingsDistributionTooLong extends Assessment {
	...
	checkTextBeforeFirstSubheadingLength (foundSubheadings) {
		...
		if (0 < foundSubheadings.length && '' === foundSubheadings[0].subheading && '' !== foundSubheadings[0].text) {
	...
	getResult (paper, researcher) {
		...
		// Only call it "Missing subheadings" when the post genuinely has none. When subheadings exist but a
		// section is over the recommended length, the issue is section length — not a missing subheading.
		assessmentResult.setTitle((7 <= calculatedResult.score || this._hasSubheadings) ? __('Subheading distribution', td) : __('Missing subheadings', td))
		...
		return {
			beginning : useCharacter => {
				const wordFeedback = sprintf(
					...
					_n(
						'Your post\'s opening section is over %1$d word with no subheading. Add a subheading to break it up.',
						'Your post\'s opening section is over %1$d words with no subheading. Add a subheading to break it up.',
				...
				const characterFeedback = sprintf(
					/* translators: %1$d expands to the recommended maximum length of a text without subheading. */
					_n(
						'Your post\'s opening section is over %1$d character with no subheading. Add a subheading to break it up.',
						'Your post\'s opening section is over %1$d characters with no subheading. Add a subheading to break it up.',
						this._config.parameters.recommendedMaximumLength,
						td
					),
					this._config.parameters.recommendedMaximumLength
			...
			nonBeginning : useCharacter => {
				const wordFeedback = sprintf(
					...
					_n(
						'%1$d section of your post is over %2$d words. Add another subheading to break it up.',
						'%1$d sections of your post are over %2$d words. Add more subheadings to break them up.',
				...
				const characterFeedback = sprintf(
					/* translators: %1$d expands to the number of sections that are too long, %2$d expands to the recommended maximum length of a text without subheading. */
					_n(
						'%1$d section of your post is over %2$d characters. Add another subheading to break it up.',
						'%1$d sections of your post are over %2$d characters. Add more subheadings to break them up.',
						this._tooLongTextsNumber,
						td
					),
					this._tooLongTextsNumber,
	...
	calculateResultForLongTextWithoutSubheadings (textBeforeFirstSubheading) {
		...
		if (this._hasSubheadings) {
			if (textBeforeFirstSubheading.isLong && 2 > this._tooLongTextsNumber) {
				/*
			...
			if (textBeforeFirstSubheading.isVeryLong && 2 > this._tooLongTextsNumber) {
				/*
			...
			if (longestSubheadingTextLength <= this._config.parameters.slightlyTooMany) {
				...
				return {
					...
					hasMarks   : false,
					resultText : __(
						'Your post is well-organized with subheadings.',
						td
					)
				}
			}
		...
		return {
			...
			hasMarks   : false,
			resultText : __(
				'Your post is long but has no subheadings. Add a few to break it into sections — readers (and search engines) follow long posts much more easily that way.',
				td
			)
		}
	}
