...
export default class WordComplexityAssessment extends Assessment {
	...
	getResult (_paper, researcher) {
		...
		assessmentResult.setTitle(7 <= calculatedScore.score ? __('Word complexity', td) : __('Complex words', td))
	...
	calculateResult () {
		...
		const hasMarks = 0 < complexWordsPercentage
		const { goodAmount, acceptableAmount } = this.getFeedbackStrings()

		if (10 > complexWordsPercentage) {
			return {
	...
	getFeedbackStrings () {
		if (!this._config.callbacks.getResultTexts) {
			return {
				acceptableAmount : __('Your post uses some long or uncommon words. Hover over highlighted words to see simpler alternatives where they fit.', td),
				goodAmount       : __('Your post uses mostly familiar words — easy to read.', td)
			}
		}
