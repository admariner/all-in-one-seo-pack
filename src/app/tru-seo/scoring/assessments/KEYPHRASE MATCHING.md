...
This document explains what we consider as a keyphrase match, and how that may differ depending on factors like site language.

## How does site language affect keyphrase matching?

There are three behaviors that depend on the site language:
...
* **Whether synonyms of the keyphrase are supported**. Keyphrase synonyms (e.g. keyphrase: 'bike', synonym: 'bicycle') added by the user are considered in some of the keyphrase-related assessments.
...
Below is an overview of which behaviors occur in which languages.
...
All users can also add **synonyms** of their keyphrase. Some of the assessments will then not only consider the keyphrase, but also its synonyms.

### Group 2: Languages with function word support

Function word support is available for many languages in the TruSEO analysis system.

In addition to per-word matching and synonym support, we also filter out **function words** from the keyphrase during keyphrase matching. For example, if the keyphrase is "how to decorate a room", we filter out the words 'how', 'to', and 'a', and only leave out the meaningful words 'decorate' and 'room'. This means that if we look, for example, if the keyphrase has been used in the introduction of the text, we are only looking for the words 'decorate' and 'room'.

### Group 3: Languages with word form support

Word form support (morphology) is available for select languages.

For languages with word form support, users have access to **full word form support**. This means that in addition to all other functionalities mentioned above, **all possible forms of their keyphrase and all possible forms of their synonyms** are recognized. For example, if the keyphrase is "room decorating ideas" and the synonym is "apartment decorating ideas" all the possible forms of each word are recognized: 'apartments', 'rooms', 'room's', 'decorate', 'decorates', 'decorated', 'idea', etc. These are then taken into account during keyphrase matching, after possible function words have been filtered out.

### Summary

| Language Group | Features Available |
|------------	|------------------	|
| All languages | Per-word matching + synonyms |
| Languages with function word support | The above + filtering out function words |
| Languages with word form support | The above + word forms of keyphrase and synonyms |
...
* Check the [documentation on scoring specific assessments](SCORING%20SEO.md) for information about which assessments use synonyms, and whether any additional matching criteria apply.
