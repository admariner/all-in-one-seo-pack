...
/**
 * Represents the assessment that checks if there are keyword or synonyms in the alt attributes of images.
 */
export default class KeyphraseInImagesAssessment extends Assessment {
	 ...
	 * @param {number} [config.parameters.lowerBoundary] The lower boundary for the number of keyword matches.
	 * @param {number} [config.parameters.upperBoundary] The upper boundary for the number of keyword matches.
	 * @param {Object} [config.scores] The scores to use for the assessment.
	 * @param {number} [config.scores.withAltGoodNumberOfKeywordMatches] The score to return if there is a good number of keyword matches.
	 * @param {number} [config.scores.withAltTooFewKeywordMatches] The score to return if there are too few keyword matches.
	 * @param {number} [config.scores.withAltTooManyKeywordMatches] The score to return if there are too many keyword matches.
	 * @param {number} [config.scores.withAltNonKeyword] The score to return if there are alt attributes without keyword.
	 * @param {number} [config.scores.noAlt] The score to return if there are no alt attributes.
	 * @param {number} [config.scores.noImagesOrKeyphrase] The score to return if there are no images or no keyword.
	 */
	constructor (config = {}) {
		super()
	 ...
	 */
	hasTooFewMatches () {
		return 4 < this.imageCount && 0 < this.altProperties.withAltKeyword &&
	...
	hasGoodNumberOfMatches () {
		return ((5 > this.imageCount && 0 < this.altProperties.withAltKeyword) ||
			(5 === this.imageCount && inRangeStartEndInclusive(this.altProperties.withAltKeyword, 2, 4)) ||
			(4 < this.imageCount &&
	 ...
	 */
	hasTooManyMatches () {
		return 4 < this.imageCount && this.altProperties.withAltKeyword > this._maxNumberOfKeyphraseMatches
	}
	...
	hasNoKeyphraseMatches () {
		return 0 < this.altProperties.withAltNonKeyword && 0 === this.altProperties.withAltKeyword
	}
	...
	calculateResult (paper) {
		...
		if (!paper.hasKeyword() || 0 === this.imageCount) {
			...
			return {
				...
				resultTitle : __('Keyword in image alt attributes', td),
				resultText  : __(
					'Add a focus keyword and at least one image to enable this check.',
					td
				)
			}
		}
		...
		if (this.hasNoKeyphraseMatches()) {
			return {
				...
				resultTitle : __('Keyword in image alt attributes', td),
				resultText  : __(
					'None of your images have alt text that includes your keyword. Add it where it genuinely describes the image. This also helps your images rank in image search and aids accessibility.',
					td
				)
			}
		}
		...
		if (this.hasTooFewMatches()) {
			return {
				...
				resultText  : sprintf(
					...
					_n(
						'Only %1$d of your %2$d images has alt text that mentions your keyword. Add your keyword to the alt text of more images where it fits naturally.',
						'Only %1$d of your %2$d images have alt text that mentions your keyword. Add your keyword to the alt text of more images where it fits naturally.',
		...
		if (this.hasGoodNumberOfMatches()) {
			return {
				...
				resultTitle : __('Keyword in image alt attributes', td),
				resultText  : __(
					'Your image alt text mentions your keyword in the right amount.',
					td
				)
			}
		}
		...
		if (this.hasTooManyMatches()) {
			return {
				...
				resultText  : sprintf(
					/* translators: %1$d expands to the number of images containing an alt attribute with the keyphrase, %2$d expands to the total number of images. */
					__(
						'%1$d of your %2$d images include your keyword in the alt text — that\'s more than needed. Only include it where it genuinely describes the image.',
						td
					),
					this.altProperties.withAltKeyword,
					this.imageCount
		...
		return {
			...
			resultTitle : __('Keyword in image alt attributes', td),
			resultText  : __(
				'Your image alt text doesn\'t mention your keyword. Add it where it genuinely describes the image. This also helps your images rank in image search and aids accessibility.',
				td
			)
		}
	}
