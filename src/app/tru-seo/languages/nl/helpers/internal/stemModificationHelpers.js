import { exceptionListHelpers } from '@/app/tru-seo/languageProcessing'
const { checkIfWordEndingIsOnExceptionList, checkIfWordIsOnListThatCanHavePrefix } = exceptionListHelpers
...
const doesPrecedingSyllableContainDiphthong = function (word, noVowelDoublingRegex) {
	return -1 === (word.search(new RegExp(noVowelDoublingRegex)))
}
...
export function modifyStem (word, modificationGroup) {
	const neededReplacement = modificationGroup.find(replacement => -1 !== word.search(new RegExp(replacement[0])))
	if ('undefined' !== typeof neededReplacement) {
