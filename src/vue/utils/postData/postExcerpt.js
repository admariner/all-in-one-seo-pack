 ...
 */
// Keep in sync with the server's getDescriptionFromContent(), which trims the
// content to this many words (wp_trim_words). Without the trim the auto-excerpt
// is the entire post body, so the meta-description counter reads far longer than
// what actually gets output.
const EXCERPT_MAX_WORDS = 55
...
const excerptFromContent = (content) => {
	...
	return EXCERPT_MAX_WORDS < words.length ? words.slice(0, EXCERPT_MAX_WORDS).join(' ') : content
}
